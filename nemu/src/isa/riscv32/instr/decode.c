...
static def_DHelper(J){
  ...
  word_t imm = (s->isa.instr.j.imm19_12 << 12) | (s->isa.instr.j.imm10_1) << 1 | \
                  (s->isa.instr.j.imm11 << 11) | (s->isa.instr.j.simm20 << 20);
...
static def_DHelper(B) {
  ...
  word_t imm = (s->isa.instr.b.simm10_5 << 5) | (s->isa.instr.b.simm4_1 << 1) | \
                  (s->isa.instr.b.simm11 << 11) | (s->isa.instr.b.simm12 << 12);
  // printf("-------------------\n");
  ...
  if((imm & 0b1000000000000) != 0){
    imm = (0xffffffff << 13) | imm;
  }
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11000 11", B     , control);
