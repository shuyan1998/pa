...
word_t paddr_read(paddr_t addr, int len) {
  ...
  if(addr >= CONFIG_MTRACE_START && addr <= CONFIG_MTRACE_END){
    Log(FMT_PADDR ": read physical memory from "FMT_PADDR" to "FMT_PADDR, cpu.pc, addr, addr + len);
  } 
#endif
...
#if defined (CONFIG_MTRACE)
  if(addr >= CONFIG_MTRACE_START && addr <= CONFIG_MTRACE_END)
    Log("write physical memory from "FMT_PADDR" to "FMT_PADDR, addr, addr + len);
#endif
